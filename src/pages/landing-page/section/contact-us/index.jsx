...
export const Contact = () => {
  ...
  return (
    <section id="kontak">
      <div className=" py-6 md:px-7 lg:py-6">
        <div className="mx-auto flex flex-col items-center gap-2 text-center">
          ...
          <div className="mt-5 flex max-h-full w-screen flex-col bg-white shadow-[-13px_4px_53px_0px_rgba(0,0,0,0.25)] md:max-h-full md:max-w-[950px] md:flex-row md:rounded-2xl xl:max-h-[420px] xl:max-w-[1000px]">
            ...
            <form
              onSubmit={onSubmit}
              className="flex w-full flex-col gap-0 px-10 py-4 md:gap-3 md:py-8"
            >
              ...
              <div>
                <div className="my-0 py-1 md:max-w-full xl:max-w-full">
                  <textarea
                    ...
                    id="message"
                    rows="4"
                    className="block w-full appearance-none border-0 border-b-2 border-gray-200 bg-transparent p-0 text-sm placeholder:text-sm focus:border-gray-500 focus:outline-none focus:ring-0"
                    type="text"
                ...
                {onTouched && errors.message && (
                  <p className="-mb-4 text-left font-sans text-xs text-red-400 sm:text-sm">
                    {errors.message}
                  </p>
                )}
              </div>
                ...
                <button
                  className={
                    Object.values(errors).some((error) => error !== '')
                      ? 'mt-4 cursor-not-allowed rounded-xl bg-primary-100/60 px-12 py-2 text-sm font-semibold text-white'
                      : 'mt-8 cursor-pointer rounded-xl bg-primary-100 px-12 py-2 text-sm font-semibold text-white hover:bg-opacity-90'
                  }
...
const FormInput = ({
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      {onTouched && error && (
        <p className="-mb-4 text-left font-sans text-xs text-red-400 sm:text-sm">{error}</p>
      )}
    </div>
  );
};
